...
| `check_config_key_consumers.py` | A declared configuration key that **reaches no consumer**. `config_key_governance.json` pins that every key is *accepted*; nothing pinned that any key is *read*, and 29 of 61 were not ([#118](https://github.com/aiperceivable/apcore/issues/118)). Every key must name what consumes it, or be recorded `inert` **with a reason** — a written decision rather than an accident. Probes execute against apcore-python and **must observe an effect outside `Config`**: four `observability.*` keys round-trip through `Config` perfectly and are consumed by nothing, so the set-then-get check anybody would write passes on all four. The `ratchet` counts may fall and may not rise. |
