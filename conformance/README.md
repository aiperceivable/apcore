...
| `config_path_typed_keys.json` | §9.2.1 | The closed set of path-typed configuration keys, declared by `"x-apcore-path": true` in `apcore-config.schema.json`. Asserts the SDK accessor equals the declared set in BOTH directions, and that `bindings.pattern` (a glob, not a path) stays out. Asserts nothing about resolution bases — that is `config_project_root.json`'s subject (§9.2.2). |
| `config_project_root.json` | §9.2.2 / §9.14 | The **project root**: the config file's directory for §9.14 discovery tiers 1-5, the process CWD for the user-level tiers 6-7 and when no file is found. One case per tier, because the tier is what selects the base. Asserts the `project_root` accessor (required now) and the narrow deprecation-warning condition — project root ≠ CWD **and** a relative path-typed value present. v1.35.0 is the DEPRECATION phase: `v1x_current_bases_unchanged` pins that 1.x resolution is unchanged, and this fixture MUST NOT gain v2.0 resolution assertions before 2.0 (#113) |
...
| `bindings_dir_resolution.json` | §5.12.6 | The binding-directory resolution contract: a loader invoked WITHOUT an explicit directory MUST resolve `bindings.dir` under §9.2 precedence (env > file > default `./bindings`) and match `bindings.pattern` through the same chain; an explicit argument still wins; and NO scan happens at client initialisation. Two discriminating cases: `config_file_dir_is_scanned_with_env_unset` (every pre-existing loader test in all three SDKs passes an explicit directory, which works under both the old and the corrected behaviour) and `env_var_must_not_be_read_directly_at_the_loader`, which sets `APCORE_BINDINGS_DIR` AFTER the Config is built so a loader reading the raw variable diverges from one reading the merged config — clause 2 had no coverage without it (#114) |
...
| `pipeline_section_wiring.json` | #118 D-72, spec v1.43.0 | §5.16 requirements 6 and 7: a `pipeline:` section in the LOADED CONFIGURATION reaches the strategy the executor runs — `remove`, `configure` and `steps` each observed on the built pipeline, plus the once-per-load diagnostic for removing `acl_check` or `approval_gate`. Drivers MUST construct the SDK's own client/executor from `input.config`: the three existing pipeline fixtures all hand the section straight to `build_strategy_from_config`, and stayed green through the whole period in which no SDK read the section from a `Config` at all |
| `tracing_from_config.json` | #118 D-68 C', spec v1.44.0 | §10.1.1: `observability.tracing.*` in the loaded configuration reaches the middleware the client runs — installation, the exporter by name, the sampling strategy and rate, the OTLP endpoint, and the two refusals (`jaeger` installs nothing and substitutes nothing; an endpoint against a non-OTLP exporter is a load-time `CONFIG_INVALID`). Drivers MUST construct the SDK's own client from `input.config`: calling the tracing-middleware builder directly proves the builder works, which was never in doubt, and passes on the implementation this fixture rules out |
| `acl_audit_delivery.json` | #118 D-66, spec v1.45.0 | §6.3.2: ACL audit DELIVERY — the one effective sink (a programmatic callback is never narrowed, levelled or silenced by the file's `audit:` block), declaration rather than the default value as the switch, the thirteen `snake_case` wire names the default sink emits, `include_denied` and its load-time notice, and the requirement that a failing sink NEVER changes the access decision. Drivers MUST load a real ACL file and drive real `check()` calls: reading the parsed block back off a config object proves the parser works, which was never the question |
| `id_map_from_config.json` | #118 D-71, spec v1.46.0 | `id_map.overrides` reaches the ID-map mechanism. The mechanism was implemented in all three SDKs and the map arrived only through a constructor argument, so the declared key renamed nothing. Drivers MUST discover from a real directory with a real `apcore.yaml` and read the registered module IDs: calling the map loader directly proves the loader works, which was never the question |
| `multi_root_discovery.json` | #118 D-70, spec v1.46.0 | `extensions.roots`: multiple extension roots WITH namespace isolation, both element shapes, mixed lists, duplicate-namespace rejection, and precedence over `extensions.root`. Two roots deriving the SAME unprefixed ID, so the prefix is observable rather than cosmetic. Drivers MUST discover from a real tree: calling `scan_multi_root` directly proves the scanner works, which was never the question in any SDK |
...
Six scripts protect the seam between the fixtures, the docs and the SDKs. Each fails on a NEW violation while accepting a recorded backlog, so they can be switched on before the backlog is closed:
...
| `check_config_key_consumers.py` | A declared configuration key that **reaches no consumer**. `config_key_governance.json` pins that every key is *accepted*; nothing pinned that any key is *read*, and 29 of 61 were not ([#118](https://github.com/aiperceivable/apcore/issues/118)). Every key must name what consumes it, or be recorded `inert` **with a reason** — a written decision rather than an accident. Probes execute against apcore-python and **must observe an effect outside `Config`**: four `observability.*` keys round-trip through `Config` perfectly and are consumed by nothing, so the set-then-get check anybody would write passes on all four. The `ratchet` counts may fall and may not rise. A key on a removal window carries a `deprecation` block naming its `since`, its `removed_no_earlier_than` and a probe; the probe asserts **both** halves — a declared key warns, and a clean configuration does not, since every one of these keys has a default and a merged-view check would warn unconditionally (PROTOCOL_SPEC 9.2.4). |
| `generate_config_key_governance.py` | An SDK default or constraint outside the key surface the canonical schemas declare. |
| `check_skip_asymmetry.py` | A spec clause **one SDK has disabled while another exercises it**. A disabled test with a written reason is honest; the failure is what happens next — the reason is a claim about the SDK, written once, and nothing re-reads it. Twice in one audit: `async_tasks.save.error.TASK_STORE_UNAVAILABLE` stayed skipped in two SDKs reading *"missing symbol TaskStoreError/TASK_STORE_UNAVAILABLE"* after **D-92 required exactly that symbol**, and `cancellation.raise_if_cancelled.*` stayed skipped in apcore-typescript after the same audit's implementation added `raiseIfCancelled()`. A skip is green, so neither turned anything red. Resolving "does symbol X exist" across three languages is guesswork; the **peer suites are the oracle**, because the three share clause ids. A baseline entry records that someone looked, **not** that the skip is correct — so an entry describing a gap that has since closed is itself reported. |
