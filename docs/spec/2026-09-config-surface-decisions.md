...
Which home survives, so that the deprecation notice can name a migration target. A notice that says "this is going away" without saying "use that instead" is half a notice, and it is what an operator writing ACL configuration gets today.
...
1. Record the surviving home in §9.2.4's table and in §9.2.4.1.
2. Amend both deprecation notices to name the survivor as the migration target.
3. Wire the survivor in all three SDKs (a 1.x change; additive).
4. Remove the other at v2.0.
...
§9.6.3 publishes a four-row behaviour matrix for an unknown top-level namespace. Measured against apcore-python, **all four rows produce the same answer**: the namespace is stored and readable through `get()`.

| `strict` | `allow_unknown` | §9.6.3 says | apcore-python does |
|---|---|---|---|
| `true` | *(ignored)* | `ConfigError` | stored (in namespace mode with `_config` at the document root it does raise) |
| `false` | `true` | stored, WARN | stored |
| `false` | `false` | **silently ignored, not stored** | **stored** |
...
The change is small and its blast radius is bounded in a way most withdrawals are not: the default is `true`, and setting `false` does nothing today, so **nobody can be relying on the current behaviour of the flag** — only a project that explicitly opted in is affected, and what they opted into is exactly what they would start getting. It also has a real use: a host that embeds apcore beside its own configuration may want the framework to ignore sections it does not own rather than hold references to them.

Against A: it is the one option that makes a `get()` start returning `null` where it returned a value. Confined to configurations that set the flag, which today is a no-op set.
...
The ID-map mechanism is implemented: apcore-python's `Registry._apply_id_map_overrides` is stage 2 of discovery and rewrites a discovered `canonical_id`; apcore-rust has the same stage. But the map arrives **only** through the `Registry(id_map_path=…)` constructor argument. Measured: with `id_map.overrides` pointing at a valid map that renames `executor/orig/mod.py`, discovery still registers `executor.orig.mod`.
...
It is a few lines — the resolution site for `extensions.root` sits beside it in the same constructor — and the capability is one a configuration file is the natural home for: which discovered file gets which module ID is a per-project fact, not a per-call-site one.
...
- **A — a declared configuration key MUST reach its mechanism from a loaded `Config`.** A facility that is API-only MUST NOT have a key in the canonical schemas.
- **B — no rule**; judge case by case, and rely on `check_config_key_consumers.py` to keep the count honest.

**Recommendation: A.**

The guard now catches this class, but it catches it *after* the key ships and only for keys someone thought to probe. A one-sentence rule in §9.1 makes the class unrepresentable, and it is the same move §9.2.3 made for pattern dialects: name the requirement once rather than fix its instances repeatedly. The four instances above are the argument for it.

**Owner**: maintainer.

**Action once decided**

- If A: add the requirement to §9.1 with a version bump, and cite it from `config_key_consumers.json`'s status definitions.
...
|---|---|---|---|
| D-66 | `acl.audit` — two homes | A (ACL file's block survives) | — |
...
| D-73 | API-only keys, in general | A (state the rule) | — |

**Suggested sequence.** D-72 first: it is the only one whose silence can cost an operator something. Then D-68's additive half (`observability.tracing.strategy`), because adding must precede removing and the v2.0 removals are already scheduled. Then D-66, so the deprecation notices can name a migration target while the window is still open. D-69, D-70 and D-71 are independent and small. D-73 last, since it is best written once the four instances have been resolved and their shape is settled.
