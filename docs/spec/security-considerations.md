...
1. **The reserved FIRST SEGMENT is enforced.** Reserved-word rejection is **not** part of Algorithm A01 (`directory_to_canonical_id`, [§2.1](./protocol-spec.md#21-directory-as-id-core-rule)) — A01 validates segment pattern and length only. What the three SDKs actually enforce in their public `register()` is a **first-segment** check against a seven-word set (`apcore-python/src/apcore/registry/registry.py:286`, `apcore-typescript/src/registry/registry.ts:204`, `apcore-rust/src/registry/registry.rs:388` — each comment says "reserved word first-segment check"). The framework reserved words are the eight in [§2.5](./protocol-spec.md#25-reserved-words): `system`, `internal`, `core`, `apcore`, `plugin`, `schema`, `acl`, `ephemeral` (`ephemeral` is handled by a separate namespace mechanism, hence seven in the SDK constant). `system.*` is therefore registrable only through the privileged `register_internal()` path ([§6.6.1](./protocol-spec.md#661-registration-restriction)), which is what this threat is about — the first segment is the only one that can carry the impersonation.

   > **Specification and implementation disagree here, and the implementations are what runs.** [§2.6 `detect_id_conflicts`](./protocol-spec.md#26-id-conflict-detection) step 2 says *"For each segment of new_id (split by "."): If segment ∈ reserved_words → conflict"*. No SDK does that: `foo.system.bar` is rejected by §2.6 as written and accepted by all three implementations. This mitigation describes the **implemented** behaviour, which is sufficient for T8 because a Canonical ID can only impersonate `system.*` by holding `system` first. The §2.6 divergence is a real spec bug, tracked in [apcore#99](https://github.com/aiperceivable/apcore/issues/99); do not read this paragraph as ratifying it.
