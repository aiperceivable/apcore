...
**The `effect` value set is closed too, at every entry point (v1.30.0, #111).** `effect` accepts `"allow"` and `"deny"` and nothing else — §6.1's field table already says **MUST** and `schemas/acl-config.schema.json` already declares the enum. A rule carrying any other value **MUST** be rejected with `ACLRuleError` naming the rule index and the offending value, and **MUST** be rejected at **every** entry point that accepts a rule — file loading, direct construction, and runtime insertion — on §6.1.6 rule 3's reasoning, which applies here unchanged. `default_effect` is closed on the same terms and at the same doors.

An implementation **MUST NOT** resolve an unrecognised `effect` to a decision. Reading it as `deny` is a fallback that looks safe and is not: under `default_effect: allow` a rule the operator wrote to permit becomes a rule that **denies everything it matches**, flipping the decision for those calls with no error and no warning, and the reading is only ever *accidentally* right on a `deny` rule — correct until someone revisits which way the fallback points. This is §6.1.5's own defect class one level down: there the key was unknown and dropped, here the key is legal and its value is dropped, and the silence is identical.

!!! danger "One guarded door is not a closed set"
    Before v1.30.0 the check existed and simply was not reached. apcore-python
    and apcore-typescript rejected an out-of-enum `effect` from `ACL.load()` and
    accepted it through direct construction and `add_rule()`, while apcore-rust
    rejected it at all three — so one ACL built in code ran under two SDKs and
    could not be constructed at all under the third, from a configuration this
    section and the schema both already called invalid. Both SDKs meanwhile
    validated `default_effect`, the same two values one field up, at every door.
    §6.1.6 rule 3 wrote the rule for this shape and it was never applied to the
...
| 1.30.0 | 2026-08-31 | **§6.1.5 — a rule's `effect` accepted any string outside the YAML loader and was silently read as `deny` (#111).** v1.27.0 closed the rule **key** set because an unknown key was dropped in silence and widened an `allow` rule. The same silence sat one level down on a legal key's **value**: §6.1's field table says `effect` **MUST** be `allow | deny` and `schemas/acl-config.schema.json` declares the enum, but nothing enforced it away from the file path. Measured across the SDKs on `effect: "Allow"` — apcore-python and apcore-typescript rejected it from `ACL.load()` and **accepted** it through direct construction and `add_rule()`, while apcore-rust rejected it at all three doors. All three emit the identical loader message, so the check existed in two SDKs and simply was not reached from two of its three entry points. One ACL built in code therefore ran under two SDKs and could not be constructed under the third. Both SDKs meanwhile validated **`default_effect`** — the same two values one field up — at every door, so the inconsistency was internal as well as cross-language. §6.1.6 rule 3 already requires rejection at file loading, direct construction and runtime insertion, and had never been applied to the field it is named after. **Not a privilege escalation** — the fallback resolves toward `deny`, so an unknown value cannot grant — but a silent functional break: under `default_effect: allow` a rule written to permit becomes one that denies everything it matches, and the reading is only accidentally right on a `deny` rule. An implementation **MUST NOT** resolve an unrecognised `effect` to a decision. `default_effect` is stated on the same terms rather than left correct-by-convention. **This IS an SDK change** in apcore-python and apcore-typescript; apcore-rust already conforms and gains a regression test. Governance: maintainer approval per GOVERNANCE.md § Decision Making; tracking issue #111. |
