---
description: "The canonical, normative apcore protocol specification (RFC 2119, v1.57.0): module, schema, naming, ACL, approval, error, config, and observability requirements for all conforming SDKs."
---
...
Steps:
  ...
  2. Reserved word detection (FIRST SEGMENT ONLY — see below):
...
annotations:
  ...
  properties:
    ...
    requires_approval:
      type: boolean
      default: false
      description: "Whether THIS MODULE requires human approval before execution. A module-level declaration, not the governance-effective value — see the note below. When an ApprovalHandler is configured, the Executor enforces this at runtime (see §7 Approval System)."
...
!!! warning "`requires_approval: false` does not mean no approval will be required (v1.29.0, #110)"
    ...
    The **governance-effective** value is call-site dependent, because the ACL condition that
    decides it reads what the call carries (§6.1.7). It is obtained from `Executor.validate()`
...
requires_approval=false → this MODULE asks for none; the CALL may still need one — preflight to find out (§7.9.5)
...
- A condition **is false** when a registered handler ran to completion and returned false, having understood the value it was given. This is an ordinary non-match: the rule does not apply to this call, and evaluation continues with the next rule.
- A condition is **unevaluable** when the implementation cannot obtain an answer to the condition **as written**.

"Unevaluable" is a **principle, not a closed list**. The situations below are the ones every implementation meets; an implementation that encounters another **MUST** classify it by the principle, and **MUST NOT** default it to UNSATISFIED because it is unlisted:
...
!!! danger "Why 4 and 5 are unevaluable and not merely false"
...
|---|---|---|
| `allow` | rule does not match → continue | rule does not match → continue (**MUST NOT** grant); a carried `approval: required` becomes **pending** (rule 5) |
...
5. **An unevaluable `allow` rule's approval requirement is pending, not discarded (v1.29.0, #109).** Rule 1's "MUST NOT grant" means the rule steps aside, which was a complete instruction while a rule carried one axis. Since §6.1.6 a rule carries two, and the second one **MUST NOT** be lost when the first resolves to unevaluable:
    ...
    - **A denial clears it.** When the final decision is `deny`, the approval requirement **MUST** be reported as `false`, and `matched_rule_index` **MUST** continue to name the rule that actually decided rather than the unevaluable one. "Denied **and** put it to a human" is the same meaningless state §6.1.6 rule 2 rejects on a single rule, and it is no more meaningful assembled from two.
...
Short-circuiting applies to **handler execution only**, never to the precheck: §6.1.4 requires the whole `conditions` tree to be examined for structural and registry faults before any handler runs, so a misspelled key or a malformed operator is found and recorded whether or not a sibling would have short-circuited past it.
...
Both are resolved by separating what can be decided **without a context and without running anything** from what cannot.

**The precheck.** Before evaluating a rule, an implementation **MUST** examine that rule's structure — including its entire `conditions` tree, every branch nested inside `$or` and `$not` — and determine:

- whether `callers` and `targets` are each a **list of strings** (§6.1.4.1);
- whether `conditions` is a mapping (§6.1.1 case 5);
- for each condition key, whether a handler is resolvable on the evaluation path in use (§6.1.1 case 1, §6.1.3);
- for each compound operator, whether its value has the required shape (§6.1.1 case 4).

The precheck is **context-independent** and **MUST NOT** invoke any handler. It examines structure and the registries only.
...
4. **The precheck decides whether a rule can be evaluated. It **MUST NOT** enlarge the set of calls a rule applies to.** A rule that a *well-formed* pattern field definitively excludes from this call does not participate in the decision, and its faults elsewhere **MUST NOT** affect it. In order:
    ...
    c. If both pattern fields are well-formed and either fails to match, the rule **does not apply to this call**. Evaluation continues with the next rule, and any fault in the rule's `conditions` **MUST NOT** be consulted, reported in `handler_error`, or allowed to change this call's decision.
    d. Only a rule whose patterns matched reaches the `conditions` precheck.
...
5. **A precheck fault gates the rule; it does not enter the composition table.** §6.1.1's table says an outright "yes" wins an `$or` even when a sibling is unevaluable. That governs conditions the implementation actually **evaluated**. A fault the precheck found is different in kind: the rule is not well-formed enough to evaluate, so it is UNEVALUABLE as a whole and no sibling can rescue it.
    ...
    Without (c), one misspelled key in a narrowly scoped rule would decide calls that rule was never written about: `callers: ["api.*"]` with `conditions: {mispelled: true}` and `effect: deny` would deny a `worker.*` caller, which breaks first-match-wins. The fault is still real — `validate_rules()` (§6.1.2) reports every fault in every rule regardless of any call, and that is where a scoped rule's typo is meant to surface.

**Determinism.** Because the precheck is context-independent, handler-free and exhaustive, its findings are a pure function of the rule. Every conforming implementation **MUST** therefore produce the **same set** of precheck-origin `handler_error` entries for the same rule, in the same order (§6.1.1 rule 2's path ordering). Diagnostics that originate in handler *execution* — a registered handler that throws, an async handler unresolvable on the sync path — carry no such guarantee, because execution may short-circuit.

**Condition paths.** A finding names the position of the offending condition in the tree, not merely its key, because a key may appear at several positions:

| Position | Path |
|---|---|
| a key `k` at the root of `conditions` | `k` |
| a key `k` inside the *i*-th element of `$or` (0-based) | `$or[i].k` |
| a key `k` inside `$not` | `$not.k` |
| the `conditions` object itself | `$` |
| the rule's `callers` / `targets` (§6.1.4.1) | `callers` / `targets` |
| an `$or` element that is not a condition object | `$or[i]` |
...
Two scoping notes, because the "if and only if" above binds to *conditions*, not to the decision:
...
| `callers` or `targets` shape is outside §6.2.1 — empty, an empty element, `$or` with no operands, `$not` with none or more than one, or a reserved token away from index 0 | Rejected with `ACLRuleError` at every entry point (§6.2.1) — `ACL`'s own constructor included, on whatever rule objects it is handed, regardless of when or how they were built; unevaluable → §6.1.4.1 only if assigned onto a rule already installed inside a live `ACL` and mutated after the fact | **MUST** |
...
!!! warning "A conditional `deny` rule does not fire for a call that carries no context"
    ...
    This row applies only to a rule that **passes §6.1.4's precheck**. The precheck
    is context-independent and runs first, so a rule that is *malformed* — a
    misspelled key, a `$or` that is not a list, a `conditions` that is not a mapping
    — is unevaluable and takes §6.1.1's path even when the call carried no context.
    The distinction is between a question this caller did not supply the input for
    (§6.5, non-match) and a question nobody can answer (§6.1.1, resolve toward
    refusing access).
...
Eight observations, each a plain fact about the executor's current state, plus one derived flag.
...
- **control modules that do NOT declare `requires_approval`, with the `approval_gate` step wired and an `ApprovalHandler` attached** — `unprotected_control_surface` MUST be `true`, because the gate returns before consulting the handler (§6.6.5.1.1);
- **the same, with `ExecutionPolicy(strict = true)` instead of a handler** — MUST also be `true`, for the same reason: `strict` never fires on a call the gate does not engage on;
- control modules that all declare `requires_approval`, gate wired, handler attached — `unprotected_control_surface` MUST be `false`;
- control modules where **some but not all** declare `requires_approval` — MUST be `true`; one ungated write module is a hole in the surface;
...
ApprovalRequest:
  type: object
  required: [module_id, arguments, context, annotations, caller_id, action]
  properties:
    module_id:
      type: string
    ...
    caller_id:
      type: string
      nullable: true
      description: "The invoking module's canonical ID, read from context.caller_id; null for a top-level/external call, exactly as Context.caller_id is (§5.7) — not the \"@external\" substitution ACL evaluation makes internally"
    action:
      type: string
      description: "The target module's canonical ID being requested, equal to module_id — a flat field so a handler can read request.action without cross-referencing request.module_id under a second name (decision D-03)"
    description:
      type: string
      nullable: true
...
A key declared in the canonical schemas **MUST** reach the mechanism it names from a
...
1. **What counts as a configuration entry point.** Three, equally: a `Config` produced by
   a **file load**, one carrying an **environment override** (§9.2), and one
   **constructed programmatically** (`Config({…})`, `set()`). A value reaching the
   mechanism from any of them satisfies this section. What does **not** satisfy it is a
   direct argument that bypasses `Config` entirely — `Registry(id_map_path=…)`,
   `build_strategy_from_config(section, …)`, `ACL(default_effect=…)`. Those arguments
...
|---|---|---|
| `extensions.root` | `APCORE_EXTENSIONS_ROOT` | `"./extensions"` |
| `extensions.roots[]` (string form) and `extensions.roots[].root` | — (list-valued; see note) | — |
...
3. **`allow_unknown: true` warns.** The namespace is stored and readable, and
   ...
   every unregistered namespace it retained, stating that they are not
   validated. The notice is a property of *this document*, never of every
   configuration — it fires only when an unregistered namespace is present —
   which is what keeps it out of the blanket-warning failure §9.2.2 rejects.

4. **An absent `_config` takes the defaults.** The matrix describes the default
   pair (`strict: false`, `allow_unknown: true`), so a document that declares an
   unregistered namespace and no `_config` block at all is requirement 3's row.
   Reading an absent block as an exemption would make the documented default
   unreachable.
...
| apcore-mcp | `tracing.strategy` |
| apcore-a2a | `tracing.strategy` |
...
1. Each entry is a regular expression in the **regex dialect** of §9.2.3, matched by
   **unanchored search**, **case-insensitively**, against a field's value. A value that
   matches is replaced by `replacement`.
2. The value rule applies to **string values only**. A number, boolean, `null`, object or
   array **MUST NOT** be tested against `regex_patterns`, and **MUST NOT** be converted to
...
1. The union of §10.6's `x-sensitive` rule and this section's two configured rules **MUST**
   be applied at **both** of the surfaces §10.6 names, using the **same** rules at each:

   a. **log emission** — the structured logger's record, including the `extra` object; and

   b. **the executor's input/output capture point** — whatever fills the context's
      `redacted_inputs` and `redacted_output`, at execution pipeline steps 3, 7 and 9.
...
Interface: Registry
   ...
   * module's descriptor as a PARSED field, so that
