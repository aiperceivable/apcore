...
> **Type:** Reference. **Normative spec:** [PROTOCOL_SPEC](../PROTOCOL_SPEC.md) (terms appear in their respective normative sections).
...
**Annotations** — Optional behavioral hints on a module (`requires_approval`, `tags`, `version`, `extra` extensible fields). Distinct from the schema's input/output type info. The wire format and `extra` semantics are normative — see [PROTOCOL_SPEC §4.4](../PROTOCOL_SPEC.md#44-annotations-extension-field-extra--wire-format).
...
**Approval Gate** — Pipeline Step 5. Invokes the registered `ApprovalHandler` when the target module declares `requires_approval=true`. Skipped entirely when no handler is configured or the annotation is absent. See [PROTOCOL_SPEC §7](../PROTOCOL_SPEC.md#7-approval-system).
...
**Canonical ID** — The dotted-path identifier for a module derived from its filesystem path (e.g. `executor.email.send_email` from `<root>/executor/email/send_email.py`). Algorithm A01 in [PROTOCOL_SPEC §2.1](../PROTOCOL_SPEC.md#2-naming-specification) is normative; A02 normalizes IDs across language casing conventions.
...
**Module** — A unit of executable behavior with an input schema, output schema, description, and an `execute()` callable. The smallest deployable artifact in apcore. See [PROTOCOL_SPEC §5](../PROTOCOL_SPEC.md#5-module-specification).
...
**PreflightResult** — The return type of `Executor.validate()` carrying per-check status, a `requires_approval` flag, and `.valid`/`.errors` properties for backward compatibility with the legacy `ValidationResult`. Six pipeline checks plus an optional module-level preflight; see [PROTOCOL_SPEC §12.8](../PROTOCOL_SPEC.md#12-sdk-implementation-guide).
...
**Schema** — JSON Schema Draft 2020-12 document describing module input or output. Specified in [PROTOCOL_SPEC §4](../PROTOCOL_SPEC.md#4-schema-specification). Three layers: **Core** (required: `input_schema`, `output_schema`, `description`), **Annotation** (optional behavioral hints), **Extension** (`x-*` open-ended).
