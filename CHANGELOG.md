...
- **A rule's `effect` accepted any string outside the YAML loader, and was silently read as `deny` (spec v1.30.0, [#111](https://github.com/aiperceivable/apcore/issues/111)).** §6.1's field table says `effect` **MUST** be `allow | deny` and `schemas/acl-config.schema.json` declares the enum, but nothing enforced it away from the file path. This is [#107](https://github.com/aiperceivable/apcore/issues/107) one level down: there the rule **key** set was closed while an unknown key was dropped in silence; here the key is legal and its **value** is dropped, in the same silence.
  ...
  **Measured on `effect: "Allow"`** — a capitalisation an operator writes by hand. apcore-python and apcore-typescript rejected it from `ACL.load()` and **accepted** it through direct construction and `add_rule()`; apcore-rust rejected it at all three doors. All three emit the identical loader message *"Rule 0 has invalid effect 'Allow', must be 'allow' or 'deny'"*, so the check existed in two SDKs and simply was not reached from two of its three entry points. One ACL built in code ran under two SDKs and could not be constructed under the third.

  **The inconsistency was internal too.** Both SDKs validate `default_effect` — the same two legal values one field up — at construction. So the loader guarded one door, `default_effect` guarded all of them, and a rule's `effect` guarded only the file path. §6.1.6 rule 3 already requires rejection at *every* entry point that accepts a rule, and had never been applied to the field it is named after.

  **Not a privilege escalation**, because the fallback resolves toward `deny` and an unknown value cannot grant. It is a silent functional break: under `default_effect: allow` a rule the operator wrote to permit becomes a rule that **denies everything it matches**, flipping those decisions with no error and no warning, and `validate_rules()` does not report it either. On a `deny` rule the fallback is only *accidentally* right — true until someone revisits which way it points. An implementation **MUST NOT** resolve an unrecognised `effect` to a decision.

  §6.1.5 now states the `effect` value closure and the entry-point requirement, with `default_effect` stated on the same terms rather than left correct-by-convention. **This IS an SDK change** in apcore-python and apcore-typescript; apcore-rust already conforms.
