...
> Ships `PROTOCOL_SPEC` **v1.37.0 → v1.39.0**. Three decisions of the same shape, one release.
...
> nobody, and the numbers that filled the gap disagreed six ways for a single field. Both are the
> same failure — a contract named but never pinned — and both close the same way: state the rule
> once, in one place, and make a fixture that can fail.
