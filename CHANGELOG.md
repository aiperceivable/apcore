...
> Ships `PROTOCOL_SPEC` **v1.37.0**. One decision, applied everywhere: the specification typed six
> values as "a glob" and named no matcher, so each SDK inherited its host library's dialect. Closing
> that needs one algorithm, one declared set, and fixture cases drawn from outside the family where
> the dialects happen to agree.
